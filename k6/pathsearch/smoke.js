...
export default function () {
  ...
  check(로그인페이지_응답, {'로그인페이지_응답 load success': (response) => response.status === 200});

  let 로그인_요청_응답 = 로그인_요청();
  check(로그인_요청_응답, {'로그인_요청_응답 load success': (response) => response.json("accessToken")  !== ""});

  let 경로검색_페이지_응답 = http.get(PATH_PAGE);
  check(경로검색_페이지_응답, {'경로검색_페이지_응답 load success': (response) => response.json("accessToken") !== ""});

  let 경로_검색_응답 = 경로검색요청(로그인_요청_응답.json("accessToken"), 1, 6);
  check(경로_검색_응답, {'경로_검색_응답 load success': (response) => response.json("accessToken") !== ""});

  let 경로_좋아요_응답 = 경로좋아요요청(로그인_요청_응답.json("accessToken"), 1,6);
  check(경로_좋아요_응답, {'경로_좋아요_응답 load success': (response) => response.json("accessToken") !== ""});
